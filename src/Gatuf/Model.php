...
class Gatuf_Model {
	...
	function setAssoc($model) {
		...
		$req .= $this->_toDb($model->$model->primary_key, $model->primary_key).')';
	...
	function delAssoc($model) {
		...
		if (!isset($this->_m['many'][$model->_a['model']])
			...
			or strlen($model->$model->primary_key) == 0) {
			return false;
		}
		$hay = array(strtolower($model->_a['model']), strtolower($this->_a['model']));
		sort($hay);
		...
		$req .= ' AND '.$this->_con->qn(strtolower($model->_a['model']).'_'.$model->primary_key).' = '.$this->_toDb($model->$model->primary_key, $model->primary_key);
		$this->_con->execute($req);
		return true;
	}
