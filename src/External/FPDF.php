...
*                                                                              *
*******************************************************************************/
function External_FPDF($orientation='P', $unit='mm', $size='A4')
{
	// Some checks
	$this->_dochecks();
...
function CellSmallCaps($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='')
{
    //Output a cell
    $txt = iconv('UTF-8', 'windows-1252', $txt);
    $abecedario = iconv ('UTF-8', 'windows-1252', "ABCDEFGHIJKLMNOPQRSTUVWXZ0123456789ÁÉÍÓÚ`!@#$%^&*()_=+|'\";:{}[]?, .<>");
    $k=$this->k;
    ...
    if($txt!=='')
    {
        ...
        if($align=='R') //right align
            $dx=$w-$this->cMargin-$this->GetStringWidth(mb_strtoupper ($txt, 'windows-1252'));
        elseif($align=='C') //center align
            $dx=($w-$this->GetStringWidth(mb_strtoupper ($txt, 'windows-1252')))/2;
        else  //for left align
            $dx=$this->cMargin;
        ...
        for ($i=0; $i<strlen($txt2); $i++) {
            ...
            if ($upper === false) { //case the letter is lowercase
                if($i==0){ //for the first char
                    ...
                    $s.=sprintf(' %.2f %.2f Td (%s) Tj', ($this->x+$dx)*$k, ($this->h-($this->y+.5*$h+.3*$this->FontSize))*$k, mb_strtoupper($letter, 'windows-1252'));
                    $s.=sprintf(' /F%d %.2f Tf ET', $this->CurrentFont['i'], $this->FontSizePt);
                } else {  // if there is no first char
                    $s.=sprintf(' BT /F%d %.2f Tf', $this->CurrentFont['i'], $this->FontSizePt*0.8);
                    $dx+=($this->GetStringWidth(mb_strtoupper (substr($txt2, $i-1, 1), 'windows-1252')))*1.02;
                    $s.=sprintf(' %.2f %.2f Td (%s) Tj', ($this->x+$dx)*$k, ($this->h-($this->y+.5*$h+.3*$this->FontSize))*$k, mb_strtoupper ($letter, 'windows-1252'));
                    $s.=sprintf(' /F%d %.2f Tf ET', $this->CurrentFont['i'], $this->FontSizePt);
                }
...
function MultiCellSmallCaps($w, $h, $txt, $border=0, $align='', $fill=0, $limit_nl=100)
{
    //Output text with automatic or explicit line breaks
    $cw=&$this->CurrentFont['cw'];
    if($w==0)
